...
export default function ElectricCars({ navigation }) {
  ...
  const filterData = (item) => {
    if (item.type.toLowerCase().includes("electric")) {
      return (
          <AllCarsContainer>
            <CarButton onPress={() => navigation.navigate("CarInfoElectric", { type: item.type, logo: item.logo, acceleration: item.acceleration, batteryCapacity: item.batteryCapacity, price: item.price, id: item.id, brand: item.brand, horsepower: item.horsepower, seats: item.seats, image: item.image, topspeed: item.topspeed, model: item.model })}>
